...
// --- STATE MANAGEMENT ---
let products =
  JSON.parse(localStorage.getItem("laCarpa_products")) || initialProducts;
let categories =
  JSON.parse(localStorage.getItem("laCarpa_categories")) || initialCategories;
let cart = [];
...
function renderProducts(items) {
  ...
  if (items.length === 0) {
    productsGrid.innerHTML =
      '<p class="w-100 text-center">No hay productos disponibles.</p>';
    return;
  }
...
window.deleteCategory = function (cat) {
  if (
    confirm(
      `¿Eliminar la categoría "${cat}"? Los productos asociados quedarán huérfanos.`,
    )
  ) {
